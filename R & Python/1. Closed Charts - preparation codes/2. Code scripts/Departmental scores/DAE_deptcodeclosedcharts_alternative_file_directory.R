...
library(dplyr)
library(readr)
closedchartsdataDAE <- read_csv("C:/Users/amri.kyaruzi/Documents/Python/emergencydept.csv")
...
##DAE
#DOCUMENTED
DAEPenultimate <- dt %>% filter(TYPE == "P" | TYPE == "N") %>% filter(SNN == 1.1 | SNN == 1.2 | SNN == 1.4 | SNN== 2.2 | SNN == 4.1 | SNN == 5.2 | SNN == 5.4 | SNN == 8.1) 
...
DEPT_TLC_AVERAGE <- (PercentTimelyDAE + PercentLegibleDAE + PercentCompleteDAE)/3

#Department of A&E Scores
