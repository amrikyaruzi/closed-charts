...
##DOA
#DOCUMENTED
DOAPenultimate <- dt %>% filter(TYPE == "P" | TYPE == "N") %>% filter(SNN == 1.3 | SNN == 4.1 | SNN == 5.1 | SNN== 5.2 | SNN == 5.3 | SNN == 5.5 | SNN == 7.2) %>% filter(DEPT == "DOA") 
...
#DEPTAVERAGE
DEPT_TLC_AVERAGE <- (PercentTimelyDOA + PercentLegibleDOA + PercentCompleteDOA)/3
