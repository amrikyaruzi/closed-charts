...
glimpse(dt)
doctorslist <- dt %>% filter(DEPT == "DOP") %>% group_by(DOCTOR) %>% count() %>% arrange(desc(n))
View(doctorslist)

##DOP
#DOCUMENTED
DOPPenultimate <- dt  %>%  filter(TYPE == "P") %>% filter(DOCTOR == "Mbise, Wilson") %>% filter(SNN == 1.1 | SNN== 2.1 | SNN == 3.2 | SNN == 5.2 | SNN == 5.4 | SNN == 7.1 |  SNN == 8.1) %>% filter(DEPT == "DOP")
...
DOCAVERAGE <- (PercentTimelyDOP + PercentLegibleDOP + PercentCompleteDOP)/3

#Doctor's scores for the Department of Paediatrics:
