...
glimpse(dt)
doctorslist <- dt %>% filter(DEPT == "DON") %>% group_by(DOCTOR) %>% count() %>% arrange(desc(n))
View(doctorslist)

##DON
#DOCUMENTED
DONPenultimate <- dt  %>%  filter(TYPE == "P" | TYPE == "N") %>% filter(DOCTOR == "Giga, Nimtaz") %>% filter(SNN == 1.5 | SNN == 2.1 | SNN == 3.1 | SNN== 3.2) %>% filter(DEPT == "DON")
...
DOCAVERAGE <- (PercentTimelyDON + PercentLegibleDON + PercentCompleteDON)/3

#Doctor's scores for the Department of Nutrition:
