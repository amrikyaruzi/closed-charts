...
glimpse(dt)
doctorslist <- dt %>% filter(DEPT == "DOM") %>% group_by(DOCTOR) %>% count() %>% arrange(desc(n))
View(doctorslist)

##DOM
#DOCUMENTED
DOMPenultimate <- dt %>%  filter(TYPE == "P") %>% filter(DOCTOR == "Robert, Mvungi" | DOCTOR == "Robert Mvungi") %>% filter(SNN == 1.1 | SNN == 1.3 | SNN== 2.1 | SNN == 3.2 | SNN == 5.2 | SNN == 5.4 | SNN == 7.1 | SNN == 7.3 |  SNN == 8.1) %>% filter(DEPT == "DOM")
...
DOCAVERAGE <- (PercentTimelyDOM + PercentLegibleDOM + PercentCompleteDOM)/3
