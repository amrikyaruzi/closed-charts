...
glimpse(dt)
doctorslist <- dt %>% filter(DEPT == "DOS") %>% group_by(DOCTOR) %>% count()
View(doctorslist)

##DOS
#DOCUMENTED
DOSPenultimate <- dt %>% filter(TYPE == "P") %>% filter(DOCTOR == "Zehri, Aliakbar") %>% filter(SNN == 1.1 | SNN== 2.1 | SNN == 3.2 | SNN == 5.2 | SNN == 5.4 | SNN == 7.1 | SNN == 7.3 |  SNN == 8.1) %>% filter(DEPT == "DOS")
...
DOCAVERAGE <- (PercentTimelyDOS + PercentLegibleDOS + PercentCompleteDOS)/3

#Doctor's scores for the Department of Surgery:
