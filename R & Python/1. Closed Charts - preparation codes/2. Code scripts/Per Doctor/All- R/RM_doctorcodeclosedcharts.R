...
doctorslist <- dt %>% filter(DEPT == "RM") %>% group_by(DOCTOR) %>% count() %>% arrange(desc(n))
View(doctorslist)

##RM
#DOCUMENTED
RMPenultimate <- dt %>% filter(TYPE == "P" | TYPE == "N") %>% filter(DOCTOR == "Aboubakar, Aisha") %>%  filter(SNN == 1.4 | SNN == 1.6 | SNN == 1.7 | SNN== 2.1 | SNN == 3.1) %>% filter(DEPT == "RM")
...
DOCAVERAGE <- (PercentTimelyRM + PercentLegibleRM + PercentCompleteRM)/3

#Doctor's scores for the Physiotherapy Department:
PercentDocumentedRM
PercentTimelyRM
PercentLegibleRM
PercentCompleteRM
