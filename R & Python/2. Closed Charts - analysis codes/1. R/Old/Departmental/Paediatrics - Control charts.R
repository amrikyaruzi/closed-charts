...
##Paediatrics
#Data
controlchart <- data1 %>% filter(DEPT == "Paediatrics")
...
#Data for tables
tables <- data1
tables <- tables %>% filter(DEPT == "Paediatrics")
...
read_docx() %>%
  body_add_par("PAEDIATRICS DEPARTMENT", style = "centered") %>%
