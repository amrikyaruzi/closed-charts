...
##Physiotherapy
#Data
controlchart <- data1 %>% filter(DEPT == "Physiotherapy")
...
read_pptx() %>%
  ...
  ph_with(dml(ggobj= overall_plot),
          location = ph_location_type(type="body")) %>% 
  print('Physiotherapy - plots.pptx')
...
#Data for tables
tables <- data1
tables <- tables %>% filter(DEPT == "Physiotherapy")
...
read_pptx() %>%
  ...
  print("Physiotherapy - scores.pptx")
...
read_docx() %>%
  body_add_par("PHYSIOTHERAPY DEPARTMENT", style = "centered") %>%
