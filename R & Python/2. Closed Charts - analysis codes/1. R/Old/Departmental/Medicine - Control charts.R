...
##Medicine
#Data
controlchart <- data1 %>% filter(DEPT == "Medicine")
...
#Data for tables
tables <- data1
tables <- tables %>% filter(DEPT == "Medicine")
...
read_docx() %>%
  body_add_par("INTERNAL MEDICINE DEPARTMENT", style = "centered") %>%
