...
##Surgery
#Data
controlchart <- data1 %>% filter(DEPT == "Surgery")
...
#Data for tables
tables <- data1
tables <- tables %>% filter(DEPT == "Surgery")
...
read_docx() %>%
  body_add_par("SURGERY DEPARTMENT", style = "centered") %>%
