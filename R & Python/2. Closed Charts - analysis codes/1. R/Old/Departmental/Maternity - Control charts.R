...
##Maternity
#Data
controlchart <- data1 %>% filter(DEPT == "Maternity")
...
#Data for tables
tables <- data1
tables <- tables %>% filter(DEPT == "Maternity")
...
read_docx() %>%
  body_add_par("OBSTETRICS & GYNAECOLOGY DEPARTMENT", style = "centered") %>%
