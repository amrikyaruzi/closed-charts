...
##Anaesthesia
#Data
controlchart <- data1 %>% filter(DEPT == "Anaesthesia")
...
#Data for tables
tables <- data1
tables <- tables %>% filter(DEPT == "Anaesthesia")
...
read_docx() %>%
  body_add_par("Anaesthesia DEPARTMENT", style = "centered") %>%
